G.AddData({
name:'Tuzhibaev's dataset',
author:'Tuzhibaev',
desc:'The Tuzhibaev dataset for Legacy.',
engineVersion:1,
manifest:0,
func:function()
{
	...
	G.funcs['new game blurb']=function()
	{
		...
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('adult'))+'"></div><div class="freelabel">x50</div>','50 Adults')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('elder'))+'"></div><div class="freelabel">x10</div>','10 Elder')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('child'))+'"></div><div class="freelabel">x20</div>','20 Children')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('herb'))+'"></div><div class="freelabel">x2500</div>','2500 Herbs')+
		G.textWithTooltip('<div class="icon freestanding" style="'+G.getIconUsedBy(G.getRes('water'))+'"></div><div class="freelabel">x2500</div>','2500 Water')+
		'</div>'+
	...
	G.funcs['production multiplier']=function()
	{
		...
		if (G.getRes('population').amount>0)
		{
			...
			if (happiness>=0) mult=(Math.pow(2,happiness+1));
	...
	new G.Res({
		...
		{
			if (me.replacement) me.hidden=false; else me.hidden=false;
		}
	});
	...
	new G.Res({
		...
		tick:function(me,tick)
		{
			...
			if (me.amount>0)
			{
				...
				var productionMult=G.doFunc('production multiplier',10);
				...
				if (tick%3==0 && G.checkPolicy('disable eating')=='off')
				{
					//drink water
					var toConsume=0;
					var weights={'baby':0,'child':0,'adult':0.5,'elder':0,'sick':0,'wounded':0};
					...
					if (lacking>0)//are we out of water?
					{
						...
						{
							G.gain('happiness',-lacking*2,'no water');
							//die off
					...
					else if (G.has('joy of eating')) {consumeMult*=1;happinessAdd+=1;}
					var weights={'baby':0,'child':0,'adult':1,'elder':0,'sick':0,'wounded':0};
					for (var i in weights)
					{toConsume+=G.getRes(i).amount*weights[i];}
					...
					if (lacking>0)//are we out of food?
					{
						...
						{
							G.gain('happiness',-lacking*1,'no food');
							//die off
							var toDie=(lacking/5)*0.05;
							if (G.year<1) toDie/=5;//less deaths in the first year
							var died=0;
							var weights={'baby':0,'child':0,'adult':0,'elder':1,'sick':0,'wounded':0};//the elderly are the first to starve off
							var sum=0;for (var i in weights){sum+=weights[i];}for (var i in weights){weights[i]/=sum;}//normalize
							for (var i in weights){var ratio=(G.getRes(i).amount/me.amount);weights[i]=ratio+(1-ratio)*weights[i];}
							for (var i in weights)
				...
				G.gain('happiness',-leftout*0,'no clothing');
				G.gain('health',-leftout*0,'no clothing');
				...
				G.gain('happiness',-leftout*0,'cold & darkness');
				G.gain('health',-leftout*0,'cold & darkness');
				
				//homelessness
				var homeless=Math.max(0,(me.amount)-G.getRes('housing').amount);
				if (G.has('sedentism') && me.amount>200 && homeless>0)
				{
				...
				{
					if (G.year>=100)//no deaths of old age the first 10 years
					{
						var n=randomFloor(G.getRes('elder').amount*0.0035);
						G.gain('corpse',n,'old age');
						G.lose('elder',n,'old age');
						G.gain('happiness',-n*1*deathUnhappinessMult,'death');
					...
					if (G.year>=50)//no aging adults the first 5 years
					{
						var n=randomFloor(G.getRes('adult').amount*0.00002);
						G.gain('elder',n,'-');G.lose('adult',n,'aging up');
					}
					var n=randomFloor(G.getRes('child').amount*0.5);G.gain('adult',n,'aging up');G.lose('child',n,'aging up');
					var n=randomFloor(G.getRes('baby').amount*0.5);G.gain('child',n,'aging up');G.lose('baby',n,'aging up');
					...
					if (parents>=2)//can't make babies by yourself
					{
						var born=0;
						var birthRate=1;
						if (me.amount<100) birthRate*=30;//more births if low pop
						if (me.amount<10) birthRate*=30;//even more births if very low pop
						if (G.checkPolicy('fertility rituals')=='on') birthRate*=12;
						...
						if (homeless>0 && me.amount>200) birthRate*=0.5;//harder to make babies if you have more than 15 people and some of them are homeless
						var n=randomFloor(G.getRes('adult').amount*0.003*birthRate);G.gain('baby',n,'birth');G.gain('happiness',n*100,'birth');born+=n;
						var n=randomFloor(G.getRes('elder').amount*0.0003*birthRate);G.gain('baby',n,'birth');G.gain('happiness',n*100,'birth');born+=n;
					...
					if (toChange>0)
					{
						...
						if (G.checkPolicy('flower rituals')=='on') toChange*=0.1;
						var changed=0;
						var weights={'baby':2,'child':1.5,'adult':1,'elder':2};
						if (G.checkPolicy('child workforce')=='on') weights['child']*=1;
						if (G.checkPolicy('elder workforce')=='on') weights['elder']*=1;
						if (G.year<50) weights['adult']=0;//adults don't fall sick the first 5 years
						for (var i in weights)
					...
					G.gain('happiness',-changed*1*deathUnhappinessMult,'death');
					...
					var sickHealing=0.1;
					if (G.checkPolicy('flower rituals')=='on') sickHealing*=12;
					var changed=0;
					...
					//wounds
					var toChange=0.000003;
					if (toChange>0)
					{
						if (G.year<50) toChange*=0.05;//less wounds the first 5 years
						if (me.amount<=15) toChange*=0.05;
						var changed=0;
						var weights={'baby':0.5,'child':0.3,'adult':0.2,'elder':2};
						if (G.checkPolicy('child workforce')=='on') weights['child']*=1;
						if (G.checkPolicy('elder workforce')=='on') weights['elder']*=1;
						if (G.year<50) weights['adult']=0;//adults don't get wounded the first 5 years
						for (var i in weights)
						{var n=G.lose(i,randomFloor(Math.random()*G.getRes(i).amount*toChange*weights[i]),'-');changed+=n;}
					...
					G.gain('happiness',changed*100,'recovery');
	...
	new G.Res({
		...
		tick:function(me,tick)
		{
			...
			if (G.getRes('population').amount>0)
			{
				...
				{
					var changed=0;
					var n=G.lose('corpse',randomFloor(G.getRes('corpse').amount*0.8),'necrophagy');G.gain('meat',n*50,'necrophagy');G.gain('bone',n*50,'necrophagy');changed+=n;
					if (n>0)
					{
						G.pseudoGather(G.getRes('faith'),changed);
						G.gain('health',-changed*0.01,'necrophagy');
					}
				}
				if (me.amount>0)
				{
					...
					if (graves.amount>graves.used)
					{
						...
						G.gain('happiness',amount*20,'burial');
					}
				}
			}
			if (graves.amount<graves.used)
	...
	new G.Res({
		...
		tick:function(me,tick)
		{
			if (G.getRes('population').amount>0 && tick%2==0)
			{
				...
				G.gain('happiness',me.amount*0.1,'health');
	...
	new G.Res({
		...
		{
			var amount=0;
			amount+=G.getRes('basket').amount*100;
			amount+=G.getRes('pot').amount*250;
			amount+=G.getRes('ice').amount*100;
	...
	new G.Res({
		...
		startWith:2500,
		visible:true,
		turnToByContext:{'drinking':{'health':0.1,'happiness':1}},
		tick:function(me,tick)
		{
			if (G.checkPolicy('disable spoiling')=='off')
			{
				var toSpoil=me.amount*0;
	...
	new G.Res({
		...
		icon:[8,6],
		visible:true,
		turnToByContext:{'drinking':{'health':-0.01,'happiness':-0.01}},
		tick:function(me,tick)
		{
			if (G.checkPolicy('disable spoiling')=='off')
			{
	...
	new G.Res({
		...
		icon:[3,7],
		visible:true,
		turnToByContext:{'eating':{'health':-0.03,'happiness':-0.05}},
		tick:function(me,tick)
		{
			if (G.checkPolicy('disable spoiling')=='off')
			{
	...
	new G.Res({
		...
		icon:[4,6],
		startWith:2500,
		turnToByContext:{'eating':{'health':0.05,'happiness':0.03},'decay':{'herb':0.992,'spoiled food':0.008}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[4,7],
		turnToByContext:{'eating':{'health':0.2,'happiness':0.1},'decay':{'spoiled food':1}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[5,7],
		turnToByContext:{'eating':{'health':0.03,'happiness':0.02,'bone':1},'decay':{'spoiled food':1}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[6,7],
		turnToByContext:{'eating':{'health':0.02,'happiness':0.04,'bone':1},'decay':{'cooked meat':0.992,'spoiled food':0.008}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[11,6],
		turnToByContext:{'eating':{'health':0.02,'happiness':0.05,'bone':1},'decay':{'cured meat':1,'spoiled food':0}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[5,6],
		turnToByContext:{'eating':{'health':0.02,'happiness':0.01,'bone':0.2},'decay':{'spoiled food':1}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[6,6],
		turnToByContext:{'eating':{'health':0.03,'happiness':0.03,'bone':0.2},'decay':{'cooked seafood':0.992,'spoiled food':0.008}},
		partOf:'food',
		category:'food',
	});
	new G.Res({
		...
		icon:[12,6],
		turnToByContext:{'eating':{'health':0.02,'happiness':0.04,'bone':0.2},'decay':{'cured seafood':1,'spoiled food':0}},
		partOf:'food',
		category:'food',
	});
	...
	new G.Res({
		...
		icon:[8,11],
		turnToByContext:{'eating':{'health':0,'happiness':-0.05},'decay':{'spoiled food':0.1}},
		//partOf:'food',
		category:'food',
		tick:function(me,tick)
		{
			var toLose=me.amount*0.3;//bugs don't like to stick around
	...
	new G.Res({
		name:'basket',
		desc:'Each basket stores 100 [food].//Will decay over time.',
		icon:[14,7],
		category:'misc',
		tick:function(me,tick)
		{
	...
	new G.Res({
		name:'pot',
		desc:'Each pot stores 250 [food].//Will decay slowly over time.',
		icon:[13,5],
		category:'misc',
		tick:function(me,tick)
		{
	...
	var researchWhenGathered=function(me,amount,by)
		{
			...
			{
				var mult=1;
				if (G.year<5) mult=125;//faster research the first 5 years
	...
	new G.Unit({
		...
		effects:[
			{type:'gather',context:'gather',amount:20,max:40},//,multMax:{'leather pouches':1.1}//TODO
			//{type:'gather',context:'gather',what:{'water':10,'muddy water':10},amount:10,max:30,req:{'gathering focus':'water'}},
			{type:'gather',context:'gather',what:{'water':10,'muddy water':10},amount:10,max:30},
			{type:'gather',context:'gather',what:{'herb':5,'fruit':5},amount:10,max:10,req:{'plant lore':true}},
	...
	new G.Unit({
		...
		effects:[
			...
			{type:'gather',what:{'insight':0.5},req:{'symbolism':true}},
			{type:'mult',value:12,req:{'wisdom rituals':'on'}}
		],
		req:{'speech':true},
		category:'discovery',
		priority:5,
	...
	new G.Unit({
		...
		upkeep:{'coin':0.1},
		effects:[
			{type:'gather',what:{'culture':1}},
			{type:'gather',what:{'culture':0.5},req:{'symbolism':true}},
			{type:'mult',value:13,req:{'artistic thinking':true}},
			{type:'mult',value:12,req:{'wisdom rituals':'on'}}
		],
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'stone':1},into:{'knapped tools':10},every:2,mode:'knap'},
			{type:'convert',from:{'bone':1},into:{'knapped tools':10},every:2,mode:'knap bone'},
			{type:'convert',from:{'stick':1,'stone':1},into:{'stone tools':10},every:4,mode:'stone tools'},
			{type:'convert',from:{'stick':1,'stone':1},into:{'stone weapons':10},every:4,mode:'stone weapons'},
			{type:'convert',from:{'stick':1,'stone':1},into:{'bow':10},every:5,mode:'bows'},
			{type:'convert',from:{'stick':15},into:{'basket':10},every:5,mode:'baskets'},
			{type:'mult',value:12,req:{'ground stone tools':true}}
		],
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'stone':1},into:{'statuette':10},every:2,mode:'stone statuettes'},
			{type:'convert',from:{'bone':1},into:{'statuette':10},every:2,mode:'bone statuettes'},
			{type:'convert',from:{'stone':10},into:{'cut stone':10},every:7,mode:'cut stone'},
			{type:'convert',from:{'cut stone':1},into:{'stone':90},every:2,mode:'smash cut stone'},
			{type:'convert',from:{'gems':10},into:{'gem block':10},every:7,mode:'gem blocks'},
			{type:'mult',value:12,req:{'ground stone tools':true}}
		],
		req:{'carving':true},
		category:'crafting',
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'hide':3},into:{'primitive clothes':10},every:4,mode:'sew hide clothing'},
			{type:'convert',from:{'herb':30},into:{'primitive clothes':10},every:10,mode:'sew grass clothing'},
			{type:'convert',from:{'leather':2},into:{'basic clothes':10},every:4,mode:'weave leather clothing'},
			{type:'convert',from:{'herb':50},into:{'basic clothes':10},every:10,mode:'weave fiber clothing'},
			{type:'convert',from:{'hide':1,'water':5,'salt':1,'log':0.1},into:{'leather':10},every:7,mode:'make leather'},
			{type:'convert',from:{'hide':1,'muddy water':5,'herb':10},into:{'leather':10},every:15,mode:'cheap make leather'},
		],
		req:{'sewing':true},
		category:'crafting',
	});
	...
	new G.Unit({
		...
		effects:[
			...
			{type:'gather',context:'hunt',amount:25,max:50,mode:'spear hunting'},
			{type:'gather',context:'hunt',amount:40,max:50,mode:'bow hunting'},//TODO : consuming arrows?
			{type:'function',func:unitGetsConverted({'wounded':1},0.00001,0.0003,'[X] [people] wounded while hunting.','hunter was','hunters were'),chance:1/300},
			{type:'mult',value:12,req:{'harvest rituals':'on'}}
		],
		req:{'hunting':true},
		category:'production',
	...
	new G.Unit({
		...
		effects:[
			...
			{type:'gather',context:'fish',amount:25,max:50,mode:'spear fishing'},
			{type:'gather',context:'fish',amount:40,max:50,mode:'line fishing'},
			{type:'mult',value:12,req:{'harvest rituals':'on'}}
		],
		req:{'fishing':true},
		category:'production',
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'stick':20},into:{'fire pit':10},every:1,mode:'stick fires'},
			{type:'convert',from:{'meat':1,'fire pit':0.01},into:{'cooked meat':10},every:1,repeat:10,mode:'cook'},
			{type:'convert',from:{'seafood':1,'fire pit':0.01},into:{'cooked seafood':10},every:1,repeat:10,mode:'cook'},
			{type:'convert',from:{'meat':1,'salt':1,'fire pit':0.01},into:{'cured meat':20},every:1,repeat:20,mode:'cure'},
			{type:'convert',from:{'seafood':1,'salt':1,'fire pit':0.01},into:{'cured seafood':20},every:1,repeat:20,mode:'cure'},
		],
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'clay':3,'fire pit':0.01},into:{'pot':10},every:1,repeat:2,mode:'clay pots'},
			{type:'convert',from:{'mud':10,'fire pit':0.01},into:{'pot':10},every:2,mode:'mud pots'}
		],
		req:{'pottery':true},
		category:'crafting',
	});
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'clay':1},into:{'brick':100},every:2,mode:'bricks'},
		],
		gizmos:true,
		req:{'masonry':true},
		category:'crafting',
	...
	new G.Unit({
		...
		upkeep:{'coin':0.1},
		effects:[
			{type:'gather',context:'dig',amount:10,max:10},
	...
	new G.Unit({
		...
		modes:{
			'off':G.MODE_OFF,
			'quarry':{name:'Quarry stone',icon:[0,8],desc:'Produce [cut stone] and other minerals.',use:{'worker':1,'stone tools':1}},
			'advanced quarry':{name:'Advanced quarry stone',icon:[8,12,0,8],desc:'Produce [cut stone] and other minerals at a superior rate with metal tools.',use:{'worker':1,'metal tools':1}},
		},
		effects:[
			{type:'gather',context:'quarry',amount:50,max:100,every:1,mode:'quarry'},
			{type:'gather',context:'quarry',what:{'cut stone':10},max:50,notMode:'off'},
			{type:'gather',context:'mine',amount:0.05,max:0.5,notMode:'off'},
			{type:'gather',context:'quarry',amount:100,max:300,every:1,mode:'advanced quarry'},
			{type:'function',func:unitGetsConverted({'wounded':1},0.00001,0.0001,'[X] [people].','quarry collapsed, wounding its workers','quarries collapsed, wounding their workers'),chance:1/500}
		],
		gizmos:true,
		req:{'quarrying':true},
		category:'production',
	...
	new G.Unit({
		...
		modes:{
			'off':G.MODE_OFF,
			'any':{name:'Any',icon:[8,8],desc:'Mine without focusing on specific ores.',use:{'worker':1,'stone tools':1}},
			'coal':{name:'Coal',icon:[12,8],desc:'Mine for [coal] with x5 efficiency.',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
			'salt':{name:'Salt',icon:[11,7],desc:'Mine for [salt].',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
			'copper':{name:'Copper',icon:[9,8],desc:'Mine for [copper ore] with x5 efficiency.',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
			'tin':{name:'Tin',icon:[13,8],desc:'Mine for [tin ore] with x5 efficiency.',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
			'iron':{name:'Iron',icon:[10,8],desc:'Mine for [iron ore] with x5 efficiency.',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
			'gold':{name:'Gold',icon:[11,8],desc:'Mine for [gold ore] with x5 efficiency.',req:{'prospecting':true},use:{'worker':1,'metal tools':1}},
		},
		effects:[
			{type:'gather',context:'mine',amount:100,max:300,mode:'any'},
			{type:'gather',context:'mine',what:{'stone':100},max:300,notMode:'off'},
			{type:'gather',context:'mine',what:{'coal':500},max:300,mode:'coal'},
			{type:'gather',context:'mine',what:{'salt':500},max:300,mode:'salt'},
			{type:'gather',context:'mine',what:{'copper ore':500},max:300,mode:'copper'},
			{type:'gather',context:'mine',what:{'tin ore':500},max:300,mode:'tin'},
			{type:'gather',context:'mine',what:{'iron ore':500},max:300,mode:'iron'},
			{type:'gather',context:'mine',what:{'gold ore':500},max:300,mode:'gold'},
			{type:'function',func:unitGetsConverted({'wounded':1},0.00001,0.0001,'[X] [people].','mine collapsed, wounding its miners','mines collapsed, wounding their miners'),chance:1/500}
		],
		gizmos:true,
		req:{'mining':true},
		category:'production',
	...
	new G.Unit({
		...
		modes:{
			'off':G.MODE_OFF,
			'copper':{name:'Copper smelting',icon:[9,9],desc:'Cast [soft metal ingot]s out of 5 [copper ore]s each.',use:{'worker':1,'stone tools':1},req:{}},
			'tin':{name:'Tin smelting',icon:[9,9],desc:'Cast [soft metal ingot]s out of 10 [tin ore]s each.',use:{'worker':1,'stone tools':1},req:{}},
			'iron':{name:'Iron smelting',icon:[10,9],desc:'Cast [hard metal ingot]s out of 5 [iron ore]s each.',use:{'worker':1,'metal tools':1},req:{'iron-working':true}},
			'gold':{name:'Gold smelting',icon:[11,9],desc:'Cast [precious metal ingot]s out of 5 [gold ore]s each.',use:{'worker':1,'metal tools':1},req:{'gold-working':true}},
			'bronze':{name:'Bronze alloying',icon:[10,9],desc:'Cast [hard metal ingot]s out of 8 [copper ore]s and 2 [tin ore]s each.',use:{'worker':1,'metal tools':1},req:{'bronze-working':true}},
			'steel':{name:'Steel alloying',icon:[12,9],desc:'Cast [strong metal ingot]s out of 19 [iron ore]s and 1 [coal] each.',use:{'worker':1,'metal tools':1},req:{'steel-making':true}},
		},
		effects:[
			{type:'convert',from:{'copper ore':5},into:{'soft metal ingot':10},repeat:6,mode:'copper'},
			{type:'convert',from:{'tin ore':10},into:{'soft metal ingot':10},repeat:6,mode:'tin'},
			{type:'convert',from:{'iron ore':5},into:{'hard metal ingot':10},repeat:6,mode:'iron'},
			{type:'convert',from:{'gold ore':5},into:{'precious metal ingot':10},repeat:2,mode:'gold'},
			{type:'convert',from:{'tin ore':2,'copper ore':8},into:{'hard metal ingot':10},repeat:6,mode:'bronze'},
			{type:'convert',from:{'iron ore':19,'coal':1},into:{'strong metal ingot':10},repeat:2,mode:'steel'},
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'soft metal ingot':2},into:{'metal tools':10},repeat:6,mode:'metal tools'},
			{type:'convert',from:{'hard metal ingot':1},into:{'metal tools':30},repeat:6,mode:'hard metal tools'},
			{type:'convert',from:{'precious metal ingot':10},into:{'gold block':10},mode:'gold blocks'},
			{type:'waste',chance:0.0001/1000},
	...
	new G.Unit({
		...
		upkeep:{'coin':0.1},
		effects:[
			{type:'gather',context:'chop',amount:10,max:10}
		],
	...
	new G.Unit({
		...
		effects:[
			{type:'convert',from:{'log':1},into:{'lumber':30},repeat:4,mode:'lumber'},
			{type:'waste',chance:0.0001/1000},
		],
		gizmos:true,
		req:{'carpentry':true},
	...
	new G.Unit({
		...
		upkeep:{'coin':0.2},
		effects:[
			{type:'gather',what:{'faith':1,'happiness':2}},
	...
	new G.Unit({
		...
		upkeep:{'coin':0.2},
		effects:[
			{type:'convert',from:{'sick':1,'herb':2.5},into:{'adult':10},chance:1,every:2},
			{type:'convert',from:{'wounded':1,'herb':2.5},into:{'adult':10},chance:1,every:2},
		],
		req:{'healing':true},
		category:'spiritual',
		priority:5,
	...
	new G.Unit({
		...
		upkeep:{'coin':0.5},
		effects:[
			{type:'gather',what:{'influence':1}},
			{type:'gather',what:{'influence':0.5},req:{'code of law':true}}
		],
	...
	new G.Unit({
		...
		effects:[
			...
			{type:'function',func:function(me){
				...
				{
					var toDie=Math.min(me.amount,randomFloor(buried*0.01));
	...
	new G.Unit({
		...
		//require:{'worker':3,'stone tools':3},
		effects:[
			{type:'provide',what:{'added material storage':40000}},
			{type:'waste',chance:0.0001/1000}
		],
	...
	new G.Unit({
		...
		//require:{'worker':2,'stone tools':2},
		effects:[
			{type:'provide',what:{'added food storage':40000}},
			{type:'waste',chance:0.0001/1000}
		],
	...
	new G.Unit({
		...
		use:{'worker':1},
		effects:[
			{type:'explore',explored:1,unexplored:0.1},
			{type:'function',func:unitGetsConverted({},0.0001,0.0005,'[X] [people].','wanderer got lost','wanderers got lost'),chance:1/1000}
		],
		req:{'speech':true},
	...
	new G.Unit({
		...
		staff:{'stone tools':1},
		effects:[
			{type:'explore',explored:0.01,unexplored:0.1},
			{type:'function',func:unitGetsConverted({},0.0001,0.0005,'[X] [people].','scout got lost','scouts got lost'),chance:1/3000}
		],
		req:{'scouting':true},
		category:'exploration',
	});
	...
	new G.Unit({
		...
		wideIcon:[0,14],
		cost:{'basic building materials':100},
		costPerStep:{'basic building materials':20,'precious building materials':2},
		steps:100,
		...
		finalStepCost:{'population':10},
		finalStepDesc:'To complete the Mausoleum, 10 of your [population,People] must be sacrificed to accompany you as servants in the afterlife.',
		use:{'land':1},
	...
	new G.Unit({
		name:'auto nanny',
		desc:'@generates 500 [fruit], 500 [cooked meat,Meat], and 1000 [water]<>Keeps your people fed so you don\'t have to.//Powered by strange energies.',
		icon:[4,2],
		cost:{},
		effects:[
			{type:'gather',what:{'fruit':500,'cooked meat':500,'water':1000}}
		],
		category:'debug',
	});
	new G.Unit({
		name:'auto brain',
		desc:'@generates 500 of [insight], [culture], [faith], [science] and [influence]<>Educates your people so you don\'t have to.//Powered by strange energies.',
		icon:[5,2],
		cost:{},
		effects:[
			{type:'gather',what:{'insight':500,'culture':500,'faith':500,'science':500,'influence':500}}
		],
		category:'debug',
	});
	...
	new G.Policy({
		...
		desc:'Improves birth rate by 11 times. Consumes 1 [faith] every 20 days; will stop if you run out.',
		icon:[8,12,2,3],
		cost:{'faith':1},
		startMode:'off',
		req:{'ritualism':true},
	...
	new G.Policy({
		...
		desc:'Improves [gatherer], [hunter] and [fisher] efficiency by 11 times. Consumes 1 [faith] every 20 days; will stop if you run out.',
		icon:[8,12,4,7],
		cost:{'faith':1},
		startMode:'off',
		req:{'ritualism':true},
	...
	new G.Policy({
		name:'wisdom rituals',
		desc:'Improves [dreamer] and [storyteller] efficiency by 11 times. Consumes 1 [faith] every 20 days; will stop if you run out.',
		icon:[8,12,8,5],
		cost:{'faith':1},
		startMode:'off',
		req:{'ritualism':true},
